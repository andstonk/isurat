alter table public.videos
  ...
  add column if not exists translation_font_family text not null default 'Arial',
  add column if not exists translation_text_color text not null default '#D8CEFF',
  add column if not exists translation_font_size integer not null default 19,
  add column if not exists translation_bold boolean not null default false,
  add column if not exists translation_italic boolean not null default false,
  add column if not exists translation_underline boolean not null default false,
  add column if not exists translation_strikethrough boolean not null default false,
  add column if not exists translation_backdrop_color text not null default '#000000',
  add column if not exists translation_backdrop_opacity integer not null default 82,
...
alter table public.videos
  ...
  add constraint videos_translation_font_family_check check (translation_font_family in (
    'Arial', 'Helvetica', 'Verdana', 'Tahoma', 'Trebuchet MS',
    'Times New Roman', 'Georgia', 'Segoe UI', 'Calibri', 'Courier New'
  )),
  drop constraint if exists videos_translation_text_color_check,
  add constraint videos_translation_text_color_check check (translation_text_color ~ '^#[0-9A-Fa-f]{6}$'),
  drop constraint if exists videos_translation_font_size_check,
  add constraint videos_translation_font_size_check check (translation_font_size between 12 and 64),
  drop constraint if exists videos_translation_backdrop_color_check,
  add constraint videos_translation_backdrop_color_check check (translation_backdrop_color ~ '^#[0-9A-Fa-f]{6}$'),
  drop constraint if exists videos_translation_backdrop_opacity_check,
  add constraint videos_translation_backdrop_opacity_check check (translation_backdrop_opacity between 0 and 100),
