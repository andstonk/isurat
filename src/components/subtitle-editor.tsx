...
/**
 * Common non-speech markers, inserted as literal text into the cue. They are plain `[...]` /
 * `♪` conventions rather than a custom syntax, so they survive export to SRT/VTT and are already
 * understood by every other captioning tool.
 */
const MARKER_PRESETS = ["♪", "[MUSIC]", "[APPLAUSE]", "[LAUGHTER]", "[INAUDIBLE]"] as const;
...
export function SubtitleEditor({ videoId }: { videoId: string }) {
  ...
  /**
   * Appends a non-speech marker to a cue's text. The word array is extended in the same step —
   * otherwise `timedWordsForCue` would see words that no longer concatenate to the text, throw
   * away the real Soniox timings, and silently break karaoke for that cue.
   */
  function appendMarker(index: number, marker: string) {
    doc.set((current) => ({
      ...current,
      cues: current.cues.map((cue, cueIndex) => {
        ...
        // Overlapping the final word is deliberate: a marker has no spoken duration of its own,
        // and it is excluded from karaoke highlighting anyway.
        const start = Math.min(last?.start_ms ?? cue.start_ms, cue.end_ms - 1);
        return { ...cue, text: `${cue.text.trim()} ${marker}`, words: [...words, { text: ` ${marker}`, start_ms: start, end_ms: cue.end_ms }] };
      }),
    }), { immediate: true });
    setMessage(`Added ${marker} to subtitle ${index + 1}. Save to keep changes.`);
  }
  ...
  return <AppShell><div className="editor-page"><div className="editor-toolbar"><div><span className="section-label">SUBTITLE EDITOR</span><h1>{video?.file_name ?? "Project"}</h1></div><div className="toolbar-actions"><span role="status">{message || fontMessage}</span>{hasTranslation && <label className="track-mode-control">Subtitle track<select value={subtitleMode} onChange={(event) => setSubtitleMode(event.target.value as SubtitleExportMode)}><option value="original">Original only</option><option value="translated">{targetLanguage} only</option><option value="bilingual">Double subtitles</option></select></label>}<button className="secondary-button" onClick={doc.undo} disabled={!doc.canUndo} title="Undo (Ctrl+Z)">↺ Undo</button><button className="secondary-button" onClick={doc.redo} disabled={!doc.canRedo} title="Redo (Ctrl+Y)">↻ Redo</button><button className="secondary-button" onClick={save}>Save changes</button><button className="secondary-button" onClick={() => setHistoryOpen(true)} title="Save and restore snapshots of this project">⟲ History</button><button className="secondary-button" onClick={() => setShareOpen(true)} title="Create a read-only link to this project">⇗ Share</button><input ref={importInputRef} type="file" accept=".srt,.vtt,text/vtt,application/x-subrip" style={{ display: "none" }} onChange={async (event) => { const file = event.target.files?.[0]; event.target.value = ""; if (file) await importSubtitleFile(file); }} /><button className="secondary-button" onClick={() => importInputRef.current?.click()} title="Import an SRT or VTT file, replacing the current subtitles">↑ Import</button>{["srt", "vtt", "txt"].map((format) => <button key={format} className="primary-button small" onClick={() => download(format)}>↓ {format.toUpperCase()}</button>)}</div></div>
    <div className="editor-workspace"><section className="preview-pane"><div className="player-wrap">{video?.playbackUrl ? <video ref={videoRef} controls preload="metadata" src={video.playbackUrl} onPlay={(event) => { event.currentTarget.defaultMuted = false; event.currentTarget.muted = false; if (event.currentTarget.volume === 0) event.currentTarget.volume = 1; if (playbackFrameRef.current != null) cancelAnimationFrame(playbackFrameRef.current); updatePlaybackTime(); }} onPause={stopPlaybackTimer} onEnded={stopPlaybackTimer} onSeeked={(event) => setTime(event.currentTarget.currentTime * 1000)} onTimeUpdate={(event) => setTime(event.currentTarget.currentTime * 1000)} /> : <div className="player-placeholder">Video preview</div>}<CaptionOverlay cue={activeCue} settings={settings} translationStyle={translationStyle} mode={subtitleMode} timeMs={time} /></div><p className="preview-help">{hasTranslation ? `Showing ${subtitleMode === "bilingual" ? "original and translated" : subtitleMode} subtitles. Downloads use this selection.` : "Play the video to preview your edited subtitles in context."}</p>
      ...
      <div className="subtitle-controls accessibility-controls"><div className="controls-heading"><div><span className="section-label">ACCESSIBILITY</span><h2>Speaker labels and markers</h2></div><small>Display only</small></div>
        <div className="control-grid">
          <label className="effect-toggle"><span>Speaker labels</span><input aria-label="Show speaker labels" type="checkbox" checked={settings.speaker_labels_enabled} onChange={(event) => patchSettings({ speaker_labels_enabled: event.target.checked }, true)} /></label>
          <label>Label format<select aria-label="Speaker label format" value={settings.speaker_label_style} disabled={!settings.speaker_labels_enabled} onChange={(event) => patchSettings({ speaker_label_style: event.target.value as SpeakerLabelStyle }, true)}>{SPEAKER_LABEL_STYLES.map((style) => <option key={style.value} value={style.value}>{style.label}</option>)}</select></label>
          <label>Label color<span className="color-control"><input aria-label="Speaker label color" type="color" value={settings.speaker_label_color} disabled={!settings.speaker_labels_enabled} onChange={(event) => patchSettings({ speaker_label_color: event.target.value })} /><output>{settings.speaker_label_color.toUpperCase()}</output></span></label>
          <label className="effect-toggle"><span>Sound markers</span><input aria-label="Highlight accessibility markers" type="checkbox" checked={settings.sound_markers_enabled} onChange={(event) => patchSettings({ sound_markers_enabled: event.target.checked }, true)} /></label>
          <label>Marker color<span className="color-control"><input aria-label="Accessibility marker color" type="color" value={settings.sound_marker_color} disabled={!settings.sound_markers_enabled} onChange={(event) => patchSettings({ sound_marker_color: event.target.value })} /><output>{settings.sound_marker_color.toUpperCase()}</output></span></label>
        </div>
        <p className="accessibility-help">Speaker labels come from the <b>Speaker</b> field on each subtitle — nothing detects who is talking yet, so labels are whatever you type. Markers are ordinary text: anything in <code>[brackets]</code>, <code>(parentheses)</code>, or between <code>♪</code> is tinted here and in exports, including files imported from elsewhere.{settings.speaker_labels_enabled ? " Downloaded SRT and TXT files carry the label as a prefix; VTT uses a standard <v> voice tag." : " Turn labels on to include them in downloads."}</p>
      </div>
